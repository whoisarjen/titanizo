<template>
      ...
      <section class="container mx-auto flex flex-row w-full gap-6 text-sm">
        <template v-if="categories?.data">
          <nuxt-link
            ...
            :key="category.id"
            :to="`/kategoria/${category.id}--${slugify(category.attributes.name, { lower: true })}`"
            class="pt-4 pb-3 cursor-pointer transition-colors uppercase font-medium border-b-[3px] border-transparent hover:border-rose-700 hover:text-rose-700"
            :class="{ 'text-rose-700': mainCategory === category}"
            @mouseover="mainCategory = category; mainCategory.attributes.categories?.data.length ? subCategory = mainCategory.attributes.categories.data[0] : false"
          >
    ...
    <transition>
      <nav v-if="mainCategory" class="bg-white fixed container mx-auto left-1/2 -translate-x-1/2 rounded-b" @mouseleave="mainCategory = null">
        <div v-if="mainCategory.attributes.categories?.data" class="flex">
          ...
          <section v-if="subCategory?.attributes.categories?.data" class="w-4/5 p-5">
            <nav>
              <ul>
  ...
  <transition>
    <div v-if="mainCategory" class="fixed inset-0 bg-black/60 backdrop-blur z-10" />
  </transition>
</template>
