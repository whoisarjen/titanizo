<template>
  <article v-if="props.product">
    <NuxtLink class="group flex flex-col h-full" :to="`/produkt/${props.product.id}--${slugify(props.product.attributes.name, { lower: true })}`">
      <nuxt-img loading="lazy" v-if="props.product.attributes.images?.data?.length" :src="`https://strapi.titanizo.pl${props.product.attributes.images?.data[0].attributes.formats.small.url}`" class="w-4/5 aspect-square object-contain mx-auto mix-blend-darken py-8" />
