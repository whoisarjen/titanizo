<template>
    <article class="container mx-auto flex pt-5 gap-10" v-if="product">
        <section class="w-full lg:w-3/4">
            <section class="border flex p-2 bg-white">
                <div class="basis-2/5 aspect-square">
                    <div class="aspect-square w-4/5 mx-auto" v-if="product.data.attributes.images?.data?.length">
                        <nuxt-img :src="`https://strapi.titanizo.pl${ product.data.attributes.images?.data[0].attributes.formats.thumbnail.url}`" class="w-full h-full object-contain"></nuxt-img>
                    </div>
                </div>
                <div class="basis-3/5 pt-10">
                    <h1 class="text-2xl uppercase tracking-tighter font-semibold  mb-1 group-hover:underline break-word">{{ product.data.attributes.manufacturer.data.attributes.name  }} {{ product.data.attributes.collection }} {{ product.data.attributes.name }} </h1>
                    <h2 class="tracking-tight">{{ product.data.attributes.name }}</h2>
                    <h4 class="text-sm"><strong>Kod producenta:</strong> {{ product.data.attributes.manufacturer_id }}</h4>
                    <p class="text-sm tracking-tight leading-snug mt-4 line-clamp-4">{{ product.data.attributes.description }}</p>
